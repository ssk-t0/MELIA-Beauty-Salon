...
| Web予約フォーム | 予約ボタンから入力フォームを開き、予約内容をブラウザ内に保存 |
| 管理画面ログイン | フッターのAdminからログイン画面を表示。管理者 / スタッフのデモアカウント対応 |
| 管理ダッシュボード | 今日の予約、月間売上、顧客数、スタッフ数、売上グラフを表示 |
| 予約管理 | 予約の追加・編集・削除、検索・絞り込み、ステータス管理、CSV出力 |
| カレンダー | 月間カレンダーで予約数と日別予約を確認 |
| 顧客管理 | 顧客の追加・編集・削除、タグ、メモ、予約履歴数を管理 |
| スタッフ管理 | スタッフの追加・編集・削除、稼働状態、月間予約数・売上を表示 |
| 売上レポート | 月別売上、平均単価、キャンセル率、サービス別内訳、CSV出力 |
| 設定 | 店舗名、営業時間、定休日、予約単位、通知メールを保存 |
...
> 予約機能はブラウザのローカルストレージに保存するポートフォリオ用実装です。サーバー送信やメール通知は行いません。
